...
public class Variable {
	...
	public static void main(String[] args) {
		...
		// 선언(생성) 방법
		// 데이터타입 변수명;
		int number1; // 정수를 담을 수 있는 number1라는 이름을 가지는 메모리 공간을 생성
...
//		int int;
		...
		// Upper CamelCase : 클래스, 이터피(첫문자를 대문자로 시작)
		// Lower CamelCase " 변수, 함수, 메서드(첫 문자를 소문자로 시작
		// UPPER_SNAKE_CASE : 상수형 변수(모든 문자를 대문자로 작성)
		// lower_snake_case: 변수, 함수, 메서드 (모든 문자를 소눔자로 작성)
		
		// 상수 (Constant)
		// 초기화가 이루어지면 변ㅂ경이 불가능한 변수
		// 변수 선언 시에 데이터 타입 앞에 final 키쿼드를 붙어서 선언
		// final 데이터타입 변수명(상수명);
		final int MAX;
		MAX = 100;
...
//		MAX = 999;
		...
		// 리티럴에 이름을 부여하기 위한 용도로 상수를 많이 사용함
