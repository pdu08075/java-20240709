...
//		int1 = 2_147_483_648L;		error
		...
		// 큰 수를 다룰 때 정수형 타입: long
		
		// 정수형 int 를 연산 결과를 저장하는 데이터타입으로 사용할 때는
		...
		// 문자열 데이터 타입: 문자 하나를 저장할 수 있는 데이터 타입
		...
		// 논리형 데이터 타입: 참과 걱짓 값을 가지는 데이터 타입
		...
		// 자동 형 변환: 작은 데이터 타입의 변수를 큰 데이터 타입의 변수에 옮겨 담을 때 발생
		
		// 강제 형 변환: 큰 데이터 타입의 변수를 작은 데이터 타입의 벼수에 옮겨 담을 때 발생
...
//		System.out.println(numbers3[99]);		error
		...
		// 1. 문자열 결합
		// + 혹은 concat(문자열)으로 두 문자열 연결할 수 있음
		...
		// lastIndexOf(문자열): 찾고자 하는 문자열이 마지막 찾은 위치의 인덱스 번호를 반환
		...
		// null을 가질 수 있음(null): 아무것도 가져와지지 않은 상태
